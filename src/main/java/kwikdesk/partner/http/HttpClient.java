...
import kwikdesk.partner.KwikDesk;
import kwikdesk.partner.exception.KwikDeskException;
...
public class HttpClient {
  ...
  public static final String USER_AGENT = format("kwikdesk-java/%s", KwikDesk.VERSION);
  ...
  public String get(String url) {
    return request(RequestMethod.GET, url, new HashMap<String, String>(), null);
  }
  ...
  public String post(String url, String payload) {
    return request(RequestMethod.POST, url, new HashMap<String, String>(), payload);
  }
  ...
  protected String request(RequestMethod method, String url, Map<String, String> headers, String payload) {
    ...
    try {
      ...
      if (method.equals(RequestMethod.GET)) {
        ...
        throw new KwikDeskException(format("Unsupported Request Method [method: %s, url: %s]", method, url));
      }
      int statusCode = conn.getResponseCode(); //triggers the request
      if (statusCode < 200 || statusCode >= 300) {
        throw new KwikDeskException(url, statusCode, getResponse(conn.getErrorStream()));
      }
      return getResponse(conn.getInputStream());
    } catch (IOException ex) {
      throw new KwikDeskException(format("Failed request [url: %s]", url), ex);
    } finally {
      if (conn != null) {
        conn.disconnect();
      }
